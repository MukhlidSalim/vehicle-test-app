...
import { EMERGENCY_PROCEDURES } from '../../constants';

interface EmergencyProcViewProps {
  t: any;
  isRTL: boolean;
  onBack: () => void;
}
...
/**
 * EmergencyProcView details the exact steps a driver should take in case of an accident or mechanical failure.
 */
export const EmergencyProcView: React.FC<EmergencyProcViewProps> = ({ 
  t, 
  isRTL, 
  onBack 
}) => {
    ...
    <div className="animate-fade-in space-y-6">
      <div className="flex items-center justify-between border-b border-gray-400 pb-4">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-xl font-black text-gray-900">{t.emergency_proc_title}</h2>
        </div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-10">
        {EMERGENCY_PROCEDURES.map((guide, idx) => (
          <div 
            key={idx} 
