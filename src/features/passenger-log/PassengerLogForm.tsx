...
export const PassengerLogForm: React.FC<Props> = ({ lang, isRTL, onExit }) => {
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      {/* Header */}
      <div className="bg-gradient-to-r from-teal-800 to-emerald-700 text-white p-6 rounded-2xl shadow-2xl relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-black tracking-tight">{t('استمارة تسجيل الركاب', 'Passenger Registration Log')}</h1>
              <p className="text-teal-100 text-xs font-bold mt-0.5">{t('تسجيل النقلات اليومية للحافلات الصغيرة', 'Daily trip logging for light buses')}</p>
            </div>
          </div>
      ...
      {step === 'setup' && (
        <div className="space-y-6 animate-fade-in">
          <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-lg space-y-5">
            <h2 className="text-lg font-black text-gray-900 flex items-center gap-2 border-b border-gray-100 pb-3">
              <Calendar size={20} className="text-teal-600" />
              {t('بيانات اليوم', 'Day Information')}
            </h2>

            {/* Vehicle Plate */}
            <div>
              <label className="text-xs font-bold text-gray-500 mb-1.5 block">{t('رقم المركبة *', 'Vehicle Plate *')}</label>
              <input
                ...
                className={`w-full py-3 px-4 rounded-xl border text-sm font-bold focus:ring-2 focus:ring-teal-300 focus:border-teal-400 outline-none transition bg-gray-50 ${attemptedSubmit && !dayInfo.vehiclePlate.trim() ? 'border-red-400 bg-red-50/50' : 'border-gray-200'}`}
              />
            </div>

            {/* Driver Name */}
            <div>
              <label className="text-xs font-bold text-gray-500 mb-1.5 block">{t('اسم السائق *', 'Driver Name *')}</label>
              <input
                ...
                className={`w-full py-3 px-4 rounded-xl border text-sm font-bold focus:ring-2 focus:ring-teal-300 focus:border-teal-400 outline-none transition bg-gray-50 ${attemptedSubmit && !dayInfo.driverName.trim() ? 'border-red-400 bg-red-50/50' : 'border-gray-200'}`}
              />
            </div>

            {/* Date (auto-filled) */}
            <div>
              <label className="text-xs font-bold text-gray-500 mb-1.5 block">{t('التاريخ', 'Date')}</label>
              <input
                type="date"
                value={dayInfo.date}
                onChange={e => setDayInfo({ ...dayInfo, date: e.target.value })}
                className="w-full py-3 px-4 rounded-xl border border-gray-200 text-sm font-bold focus:ring-2 focus:ring-teal-300 focus:border-teal-400 outline-none transition bg-gray-50"
              />
            </div>

            {/* Vehicle Class */}
            <div>
              <label className="text-xs font-bold text-gray-500 mb-2 block">{t('رمز التصنيف *', 'Vehicle Class *')}</label>
              <div className="grid grid-cols-4 gap-3">
                {VEHICLE_CLASSES.map(cls => (
                  <button
                    key={cls}
                    onClick={() => setDayInfo({ ...dayInfo, vehicleClass: cls })}
                    className={`py-3.5 rounded-xl font-black text-lg border-2 transition-all duration-200 ${
          ...
          <button
            ...
            className="w-full py-4 bg-gradient-to-r from-teal-600 to-emerald-600 text-white rounded-2xl font-black text-base shadow-xl shadow-teal-600/20 hover:shadow-2xl hover:shadow-teal-600/30 hover:-translate-y-0.5 active:scale-[0.99] transition-all duration-300 flex items-center justify-center gap-3"
          >
            <Bus size={20} />
        ...
        <div className="space-y-5 animate-fade-in">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-4 shadow-lg">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[
                { label: t('المركبة', 'Vehicle'), value: dayInfo.vehiclePlate, icon: <Bus size={14} className="text-teal-600" /> },
                { label: t('السائق', 'Driver'), value: dayInfo.driverName, icon: <User size={14} className="text-blue-600" /> },
                { label: t('التصنيف', 'Class'), value: dayInfo.vehicleClass, icon: <Hash size={14} className="text-purple-600" /> },
                { label: t('التاريخ', 'Date'), value: dayInfo.date, icon: <Calendar size={14} className="text-amber-600" /> },
              ].map((item, i) => (
                <div key={i} className="bg-gray-50 rounded-xl p-3 border border-gray-100">
                  <div className="flex items-center gap-1.5 text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-0.5">{item.icon} {item.label}</div>
                  <div className="text-sm font-black text-gray-900 truncate">{item.value || '-'}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="bg-gradient-to-br from-teal-50 to-emerald-50 rounded-xl p-4 border border-teal-200 text-center">
              ...
              <div className="text-[10px] font-bold text-teal-500 uppercase tracking-wider">{t('النقلات', 'Trips')}</div>
            </div>
            <div className="bg-gradient-to-br from-blue-50 to-sky-50 rounded-xl p-4 border border-blue-200 text-center">
              <div className="text-2xl font-black text-blue-700">{totalPassengers}</div>
              <div className="text-[10px] font-bold text-blue-500 uppercase tracking-wider">{t('الركاب', 'Passengers')}</div>
            </div>
            <div className="bg-gradient-to-br from-purple-50 to-fuchsia-50 rounded-xl p-4 border border-purple-200 text-center">
              <div className="text-lg font-black text-purple-700">{routineCount}<span className="text-gray-300 mx-1">/</span>{shiftCount}</div>
              <div className="text-[10px] font-bold text-purple-500 uppercase tracking-wider">{t('روتينية/مناوبة', 'Routine/Shift')}</div>
            </div>
          </div>
          ...
          {trips.length > 0 && (
            <div className="bg-white rounded-2xl border border-gray-200 shadow-lg overflow-hidden">
              <div className="bg-gray-50 px-5 py-3 border-b border-gray-200 flex items-center justify-between">
                ...
                <span className="text-[10px] font-bold text-gray-400 bg-gray-100 px-2 py-1 rounded-lg">{trips.length} {t('نقلة', 'trips')}</span>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-50 border-b border-gray-200">
                      <th className="px-4 py-3 text-start font-black text-gray-600 w-12">#</th>
                      <th className="px-4 py-3 text-start font-black text-gray-600">{t('النوع', 'Type')}</th>
                      <th className="px-4 py-3 text-start font-black text-gray-600">{t('نقطة الانطلاق', 'Starting Point')}</th>
                      <th className="px-4 py-3 text-start font-black text-gray-600">{t('نقطة الوصول', 'Arrival Point')}</th>
                      <th className="px-4 py-3 text-center font-black text-gray-600">{t('الركاب', 'Passengers')}</th>
                      <th className="px-4 py-3 text-start font-black text-gray-600">{t('التوقيت', 'Time')}</th>
                      <th className="px-4 py-3 text-end font-black text-gray-600 w-24"></th>
                    </tr>
                  </thead>
                      ...
                      <tr key={trip.id} className="group hover:bg-gray-50/80 transition-colors">
                        <td className="px-4 py-3">
                          <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-black ${
                            trip.type === 'routine' ? 'bg-blue-100 text-blue-700' : 'bg-purple-100 text-purple-700'
                          }`}>
                            {idx + 1}
                          </div>
                        </td>
                        ...
                        <td className="px-4 py-3 text-center">
                          <div className="inline-flex items-center gap-1.5 bg-gray-100 px-3 py-1 rounded-lg text-xs font-black text-gray-700">
          ...
          {/* Add Trip Form */}
          {showAddTrip && (
            <div className="bg-white rounded-2xl border-2 border-teal-200 p-5 shadow-xl shadow-teal-500/10 space-y-4 animate-fade-in">
              <h3 className="text-base font-black text-gray-800 flex items-center gap-2 border-b border-gray-100 pb-3">
                {editingTripId ? <Edit3 size={18} className="text-amber-600" /> : <Plus size={18} className="text-teal-600" />}
                {editingTripId ? t('تعديل النقلة', 'Edit Trip') : t('إضافة نقلة جديدة', 'Add New Trip')}
              </h3>

              {/* Trip Type */}
              <div>
                <label className="text-xs font-bold text-gray-500 mb-2 block">{t('نوع النقلة *', 'Trip Type *')}</label>
                <div className="grid grid-cols-2 gap-3">
                  {TRIP_TYPES.map(tt => (
                    <button
                      key={tt.value}
                      onClick={() => setNewTrip({ ...newTrip, type: tt.value })}
                      className={`py-3 px-4 rounded-xl font-black text-sm border-2 transition-all duration-200 flex items-center justify-center gap-2 ${
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-bold text-gray-500 mb-1.5 block">{t('نقطة الانطلاق *', 'Starting Point *')}</label>
                  <select
                    value={newTrip.pickupLocation}
                    onChange={e => setNewTrip({ ...newTrip, pickupLocation: e.target.value })}
                    className={`w-full py-3 px-4 rounded-xl border text-sm font-bold focus:ring-2 focus:ring-teal-300 outline-none transition bg-gray-50 appearance-none ${
                ...
                <div>
                  <label className="text-xs font-bold text-gray-500 mb-1.5 block">{t('نقطة الوصول *', 'Arrival Point *')}</label>
                  <select
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-bold text-gray-500 mb-1.5 block">{t('عدد الركاب *', 'Passengers *')}</label>
                  <input
                    type="number"
                    min="1"
                    ...
                    placeholder="0"
                    className="w-full py-3 px-4 rounded-xl border border-gray-200 text-sm font-bold focus:ring-2 focus:ring-teal-300 outline-none transition bg-gray-50"
                  />
                </div>
                <div>
                  <label className="text-xs font-bold text-gray-500 mb-1.5 block">{t('التوقيت *', 'Time *')}</label>
                  <input
                    type="time"
                    value={newTrip.time}
              ...
              <div className="flex gap-3 pt-2">
                <button
                  ...
                  className="flex-1 py-3 bg-gray-100 text-gray-600 rounded-xl font-black text-sm hover:bg-gray-200 active:scale-95 transition-all"
                >
                  {t('إلغاء', 'Cancel')}
                </button>
                <button
                  onClick={handleSaveTrip}
                  className="flex-1 py-3 bg-gradient-to-r from-teal-600 to-emerald-600 text-white rounded-xl font-black text-sm shadow-lg hover:shadow-xl hover:-translate-y-0.5 active:scale-95 transition-all flex items-center justify-center gap-2"
                >
                  <Check size={16} />
          ...
          {!showAddTrip && (
            <button
              ...
              className="w-full py-4 border-2 border-dashed border-teal-300 text-teal-600 rounded-2xl font-black text-sm hover:bg-teal-50 hover:border-teal-400 active:scale-[0.99] transition-all flex items-center justify-center gap-2"
            >
              <Plus size={18} />
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-4 border-t border-gray-200">
            {trips.length > 0 && (
              <button
                ...
                className="flex-1 py-3.5 bg-gradient-to-r from-teal-600 to-emerald-600 text-white rounded-xl font-black text-sm shadow-lg hover:shadow-xl hover:-translate-y-0.5 active:scale-95 transition-all flex items-center justify-center gap-2"
              >
                <FileText size={16} />
            ...
            <button
              ...
              className="flex-1 py-3.5 bg-amber-50 text-amber-700 border border-amber-200 rounded-xl font-black text-sm hover:bg-amber-100 active:scale-95 transition-all flex items-center justify-center gap-2"
            >
              <RotateCcw size={16} />
            ...
            <button
              onClick={onExit}
              className="flex-1 py-3.5 bg-gray-800 text-white rounded-xl font-black text-sm hover:bg-gray-900 active:scale-95 transition-all flex items-center justify-center gap-2"
            >
              <ArrowLeft size={16} />
      ...
      {step === 'report' && (
        <div className="space-y-6 animate-fade-in">
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-4 border-t border-gray-200">
            <button
              onClick={() => setStep('log')}
              className="flex-1 py-3.5 bg-white border border-gray-300 text-gray-700 rounded-xl font-black text-sm hover:bg-gray-50 active:scale-95 transition-all flex items-center justify-center gap-2"
            >
              <ArrowLeft size={16} />
            ...
            <button
              ...
              className="flex-1 py-3.5 bg-blue-50 text-blue-700 border border-blue-200 rounded-xl font-black text-sm hover:bg-blue-100 active:scale-95 transition-all flex items-center justify-center gap-2"
            >
              <Plus size={16} />
              {t('إضافة نقلة جديدة', 'Add New Trip')}
            </button>
